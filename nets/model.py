...
@author: gw.kayak
"""
from transformer_text_encoder import text_encoder
from images_encoder import img_encoder
import torch
...
class net(nn.Module):
    def __init__(self):
        ...
        self.out.weight.data.normal_(0, 0.1)
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        self.lstm_img = nn.LSTM(1024, 1024, batch_first=True)
        self.lstm_text = nn.LSTM(1024, 1024, batch_first=True)
    ...
    def forward(self,img,text):
        ...
        img_feature=self.lstm_img(img_feature.unsqueeze(dim=0))
        text_feature=self.lstm_text(text_feature.unsqueeze(dim=0))
